...
// BEGIN EVENTS CALENDAR 1.1
	tep_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_model,products_image, 
		...
		(int)$product['manufacturers_id'] . "', '" . $product['products_event_begins'] . "', '" . 
		$product['products_event_expires'] . "', '" . $product['products_event_state'] . "')");
// END EVENTS CALENDAR 1.1
...
<script language="javascript" type="text/javascript" src="includes/javascript/datetimepicker/datetimepicker_css.js"></script>
</head>
    ...
    <tr bgcolor="#ebebff">
        ...
        <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') .
         	 tep_draw_input_field('products_event_begins', $pInfo->products_event_expires, 'id="products_event_begins"'); ?>
				<a href="javascript:NewCssCal('products_event_begins','yyyymmdd','arrow',true,24,false)"><img src="images/cal.gif" width="16" height="16" alt="Pick a date"></a>
        </td>
     </tr>
     <tr bgcolor="#ebebff">
         <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '5'); ?></td>
     ...
     <tr bgcolor="#ebebff">
         ...
         <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') .
         	 tep_draw_input_field('products_event_expires', $pInfo->products_event_expires, 'id="products_event_expires"'); ?>
				 <a href="javascript:NewCssCal('products_event_expires','yyyymmdd','arrow',true,24,false)"><img src="images/cal.gif" width="16" height="16" alt="Pick a date"></a>
      </tr>
<!-- END EVENTS CALENDAR 1.1 -->
...
// BEGIN EXPIRED PRODUCT 1.1
	$product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.products_event_begins, p.products_event_expires, p.products_event_state from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
...
<?php
	...
	// BEGIN EVENTS CALENDAR 1.1
        ...
        ?>
          <tr>
          <td align="center" class="smallText"><?php echo sprintf(TEXT_PRODUCT_EVENT_BEGINS, date('l j F, Y \\a\t H:i', strtotime($pInfo->products_event_begins))); ?></td>
          </tr>
          <tr>
          <td align="center" class="smallText"><?php echo sprintf(TEXT_PRODUCT_EVENT_EXPIRES, date('l j F, Y \\a\t H:i', strtotime($pInfo->products_event_expires))); ?></td>
          </tr>
        <?php
...
// BEGIN EVENTS CALENDAR 1.1
	$products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_event_begins, p.products_event_expires, p.products_event_state, p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and pd.products_name like '%" . tep_db_input($search) . "%' order by pd.products_name");
// END EVENTS CALENDAR 1.1

    } else {

// BEGIN EVENTS CALENDAR 1.1
	$products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_event_begins, p.products_event_expires, p.products_event_state from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$current_category_id . "' order by pd.products_name");
// END EVENTS CALENDAR 1.1
