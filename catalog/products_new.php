...
<?php
  $products_new_array = array();
	...
	$products_new_query_raw = "select p.products_id, pd.products_name, p.products_image, p.products_price, p.products_tax_class_id, p.products_date_added, m.manufacturers_name, p.products_event_date, p.products_event_expires_type from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on (p.manufacturers_id = m.manufacturers_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and (p.products_event_expires_type = 'disable' or ((p.products_event_expires_type = 'today' and p.products_event_date < '" . date("Y-m-d") . "')) or (p.products_event_expires_type = 'tomarrow' and p.products_event_date <= '" . date("Y-m-d") . "')) and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by p.products_date_added DESC, pd.products_name";
