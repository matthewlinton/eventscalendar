...
*/
  ...
  $breadcrumb->add(NAVBAR_TITLE, tep_href_link(FILENAME_EVENTS_CALENDAR));
...
$products_events_sql = "select p.products_id, pd.products_name, pd.products_description, p.products_price, p.products_tax_class_id, p.products_date_added, m.manufacturers_name, p.products_event_date, p.products_event_expires_type from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on (p.manufacturers_id = m.manufacturers_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_event_expires_type != 'disable' and p.products_event_date like '" . $month_match . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by p.products_event_date ASC, pd.products_name";

$i = 0;
$event = array();
$calendar_events = array();
(isset($HTTP_GET_VARS['print']) ? $len_name = 50 : $len_name = 10);
$products_events_query = tep_db_query($products_events_sql);
while ($products_events = tep_db_fetch_array($products_events_query)) {
	$event_date = split('-', $products_events['products_event_date']);
        $calendar_events[ltrim($event_date[2], '0')][$i] = array( tep_href_link(FILENAME_PRODUCT_INFO) . '?products_id=' .
        $products_events['products_id'],
        tep_truncate_string($products_events['products_name'], $len_name),
        '<b>' . $products_events['products_name'] . '</b><br>' .
        tep_truncate_string(strip_tags($products_events['products_description']), 100));
        $i++;
}
